...
class EventController extends AdminController
{
    ...
    public function config()
    {
        ...
        $admin_config->title('活动基本设置')->data($data)
            ->keyBool('NEED_VERIFY', '创建活动是否需要审核','默认无需审核')
            ->keyBool('SHENHE_SEND_WEIBO', '审核活动是否发新鲜事','默认不发新鲜事')->keyDefault('SHENHE_SEND_WEIBO',0)
            ->group('基本配置','NEED_VERIFY,SHENHE_SEND_WEIBO')
