...
class ForumController extends AdminController
{
    ...
    public function config()
    {
        ...
        $admin_config->title('论坛设置')
            ...
            ->keyInteger('FORM_POST_SHOW_NUM_INDEX', '论坛首页每页展示贴子数量', '')->keyDefault('FORM_POST_SHOW_NUM_INDEX', '5')
            ->keyInteger('FORM_POST_SHOW_NUM_PAGE', '每页展示贴子数量', '对前台除了首页以外的贴子列表有效,')->keyDefault('FORM_POST_SHOW_NUM_PAGE', '10')
            ...
            ->group('基础设置', 'LIMIT_IMAGE,FORUM_BLOCK_SIZE,CACHE_TIME,SUGGESTION_POSTS,HOT_FORUM,RECOMMAND_FORUM,FORM_POST_SHOW_NUM_INDEX,FORM_POST_SHOW_NUM_PAGE')
    ...
    public function editForum($id = null, $title = '', $create_time = 0, $status = 1, $allow_user_group = 0, $logo = 0,$type_id=0)
    {
        if (IS_POST) {
            ...
            $builder
                ...
                ->keySingleImage('logo', '板块图标', '建议160px*160px')
                ->keySingleImage('background', '板块背景', '板块背景图,用于显示的封面1140px*180px')
                ->keyStatus()
                ->keyCreateTime()
