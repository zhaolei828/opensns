...
class RoleController extends AdminController
{
    ...
    public function index($page = 1, $r = 20)
    {
        ...
        $builder->title("身份列表");
        $builder->buttonNew(U('Role/editRole'))->setStatusUrl(U('setStatus'))->buttonEnable()->buttonDisable()->button('删除', array('class' => 'btn ajax-post confirm', 'url' => U('setStatus', array('status' => -1)), 'target-form' => 'ids', 'confirm-info' => "确认删除身份？删除后不可恢复！"))->buttonSort(U('sort'));
        $builder->keyId()
            ->keyText('title', '身份名')
            ->keyText('name', '身份标识')
            ->keyText('group', '所属分组')
    ...
    public function userList($page = 1, $r = 20)
    {
        ...
        $builder->buttonModalPopup(U('Role/changeRole',array('role_id'=>$map_user_list['role_id'])), array(), '迁移用户',array('data-title'=>'迁移用户到其他身份','target-form'=>'ids'))
            ->button('初始化没身份的用户', array('href' => U('Role/initUnhaveUser')))
            ->select('身份：', 'role_id', 'select', '', '', '', $role_list)->select('状态：', 'user_status', 'select', '', '', '', $statusOptions)->select('', 'single_role', 'select', '', '', '', $singleRoleOptions)
            ->keyId()
            ->keyImage('avatar', '头像')
